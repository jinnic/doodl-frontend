...
class Profile extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <h2>{this.state.user.user_name}'s artwork</h2>
          <p>bio: {this.state.user.bio}</p>
          <button type="button" className="btn btn-primary" data-toggle="modal" data-target="#profileModal">edit user profile</button>
          <ProfileEditForm userDelete={this.props.userDelete} userUpdate={userUpdate} user={this.state.user} updateUserInfo={this.updateUserInfo}/>
          <DoodleContainer 
          user={user}
